...
namespace JYW.ArrowBattle.UIs
{
    public class SkillPanel : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            EventManager.Instance.CooldownUIEvent -= StartCooldown;
            EventManager.Instance.CooldownUIEvent += StartCooldown;
            ...
            for (int i = 1; i <= 5; i++) //SO 토대로 스킬 아이콘 설정
            {
                ...
                var skillEnum = System.Enum.Parse<SkillType>($"Skill{i}");

                // 아이콘 적용
                map[panelEnum].GetComponent<Image>().sprite = skillDatasSO.GetSkillDataById(skillEnum).SkillIcon;
            }
        ...
        private static SkillPanelEnum ToEnum(string name) => (SkillPanelEnum)System.Enum.Parse(typeof(SkillPanelEnum), name);
    }
